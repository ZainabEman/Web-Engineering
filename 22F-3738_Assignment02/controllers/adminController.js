...
const getDashboard = async (req, res) => {
  try {
    ...
    const lowAvailabilityCoursesCount = await Course.countDocuments({
      availableSeats: { $lt: 5 },
    });
...
const createCourse = async (req, res) => {
  ...
  try {
    ...
    // Notify clients (Socket.IO) that a new course was created
...
const updateCourse = async (req, res) => {
  ...
  try {
    ...
    const newAvailableSeats = parseInt(availableSeats, 10);
    ...
    // Notify clients that a course was updated
...
const deleteCourse = async (req, res) => {
  ...
  try {
    ...
    // Notify clients that a course was deleted
...
const getStudentDetails = async (req, res) => {
  try {
    ...
    const student = await User.findById(id).populate('enrolledCourses');
    if (!student) {
      return res.status(404).render('auth/error', {
...
const forceEnrollStudent = async (req, res) => {
  ...
  try {
    ...
    // Notify clients about seat availability update
    req.io.emit('courseUpdated', {
      courseId,
      availableSeats: course.availableSeats,
    });
  ...
  try {
    ...
    if (type === 'enrollment') {
      ...
      for (const student of students) {
        // Build a set of course IDs the student is enrolled in
        ...
        for (const course of student.enrolledCourses) {
          // Retrieve course details along with its prerequisites
          const courseWithPrereqs = await Course.findById(course._id).populate('prerequisites');

          if (courseWithPrereqs.prerequisites && courseWithPrereqs.prerequisites.length > 0) {
            // Find prerequisites that are missing from the student's enrolled courses
            const missing = courseWithPrereqs.prerequisites.filter(prereq =>
            !studentCourseIds.includes(prereq._id.toString())
            );
            ...
            if (missing.length > 0) {
              missingPrerequisites.push({
                ...
                course: courseWithPrereqs.title,
                missingPrerequisites: missing.map(pr => `${pr.courseCode} - ${pr.title}`)
              });
            }
          }
        }
      ...
      // If no missing prerequisites, pass an informative message
      if (missingPrerequisites.length === 0) {
        reportData = { message: 'All students have met their prerequisites.' };
      } else {
        reportData = missingPrerequisites;
      }
...
/**
 * Check waitlist and notify students (when seats become available)
 */
const checkWaitlistAndNotify = async (courseId, session) => {
  try {
    ...
    console.log(
      `Admin action: Notification sent to ${subscriptions.length} students for course ${courseId}`
    );
  } catch (error) {
